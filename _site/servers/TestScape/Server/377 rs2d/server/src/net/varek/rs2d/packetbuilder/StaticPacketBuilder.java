...
public class StaticPacketBuilder implements PacketBuilder {
    ...
    public StaticPacketBuilder addBytes_reverse(byte[] data) {
        ensureCapacity(curLength + data.length);
        for(int k = data.length - 1; k >= 0; k--)
            addByte(data[k]);
        return this;
    }

    public StaticPacketBuilder addBytesA(byte[] data) {
        ensureCapacity(curLength + data.length);
        for (byte aData : data) {
            addByteA(aData);
        }
    ...
    public StaticPacketBuilder addShortBigEndianA(int i) {
		ensureCapacity(curLength + 2);
        addByte((byte)(i + 128), false);
        addByte((byte)(i >> 8), false);
		return this;
	...
	public StaticPacketBuilder addShortBigEndian(int val) {
		ensureCapacity(curLength + 2);
		addByte((byte) val, false);
		addByte((byte) (val >> 8), false);
		return this;
	...
	public StaticPacketBuilder addIntBigEndian(int val) {
		ensureCapacity(curLength + 4);
		addByte((byte) val, false);
		addByte((byte) (val >> 8), false);
		addByte((byte) (val >> 16), false);
	...
	public StaticPacketBuilder addLELong(long val) {
		addIntBigEndian((int) (val & -1L));
		addIntBigEndian((int) (val >> 32));
		return this;
	}
