 ...
 // Global Item VARS
	...
	public void process()
	{
		for (int i = 0; i <= 5000; i++)
		{
			...
			if (showItemTimer == globalItemTicks[i]) // Item has expired, show to all
			{
				if(!globalItemStatic[i])
				{
	...
	{
		for (int i = 0; i <= 5000; i++) // Loop through all item spots
		{
			if(globalItemID[i] == itemID && globalItemX[i] == itemX && globalItemY[i] == itemY)	// Found item
			return true;
		}
	...
	public static void addItem(int itemID, int itemX, int itemY, int itemAmount, int itemController, boolean itemStatic)
	{
		for (int i = 0; i <= 5000; i++) // Loop through all item spots
		{
			if(globalItemID[i] == 0) // Found blank item spot
			{
	...
	public static int SDID = 90; //90 * 500 = 45000 / 1000 = 45s ... SDID = Standard Drop Items Delay
	...
	{
		int slot = -1; // First, search for a free slot
		...
		if(slot == -1) // If no free slot is found
		return;
