...
if (npcs[i].npcType == 946) {
                        if (misc.random2(30) <= 2) {
                            npcs[i].updateRequired = true;
                            npcs[i].textUpdateRequired = true;
                            npcs[i].textUpdate = "Runes runes click on me for runes";
                        }
                    }
if (npcs[i].npcType == 1694) {
                        if (misc.random2(30) <= 2) {
                            npcs[i].updateRequired = true;
                            npcs[i].textUpdateRequired = true;
                            npcs[i].textUpdate = "Range stuff here";
                        }
                    }
if (npcs[i].npcType == 1051) {
                        if (misc.random2(30) <= 2) {
                            npcs[i].updateRequired = true;
                            npcs[i].textUpdateRequired = true;
                            npcs[i].textUpdate = "I have your starter talk to me";
                        }
                    }
...
npcs[i].updateRequired = true;
npcs[i].textUpdateRequired = true;
npcs[i].textUpdate = "Rawr Project Czar!";
}
}
...
npcs[i].updateRequired = true;
npcs[i].textUpdateRequired = true;
npcs[i].textUpdate = "Go in this portal for training area!";
}
}
...
npcs[i].updateRequired = true;
npcs[i].textUpdateRequired = true;
npcs[i].textUpdate = "Go in this portal to pk at edgevill!";
}
}
...
return 1834;
		...
		if (server.playerHandler.players[Player].playerEquipment[server.playerHandler.players[Player].playerRing] == 2570) {
			RingOfLife = true;
                ...
                //if(EnemyX != npcs[NPCID].absX && EnemyY != npcs[NPCID].absY) // stops client crashing
                //plr.viewTo(npcs[NPCID].absX, npcs[NPCID].absY); // Player turns to npc

                if(server.playerHandler.players[Player].attacknpc == NPCID) {
                server.playerHandler.players[Player].faceNPC = NPCID; // sets npc index for player to view
                server.playerHandler.players[Player].faceNPCupdate = true; // updates face npc index so player faces npcs
                server.playerHandler.players[Player].attacknpc = NPCID; // makes it so if player runs away the player attacks back when npc follows
                server.playerHandler.players[Player].IsAttackingNPC = true; // makes it so if player runs away the player attacks back when npc follows
                }
		int hitDiff = 0;
