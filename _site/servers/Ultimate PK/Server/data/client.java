...
if((absX >= 3175 && absX <= 3289 && absY >= 3376 && absY <= 3457) || (absX >= 2690 && absX <= 2730 && absY >= 9726 && absY <= 9788) || (absX >= 3180 && absX <= 3186 && absY >= 3433 && absY <= 3446) || heightLevel == 1)
return true;
else
return false;
}

public boolean inclanwar() {
if((absX >= 2580 && absX <= 2606 && absY >= 3152 && absY <= 3173))
return true;
else
return false;
}

public int untradable[] = {995,84,3006,4653,4703,744,793,6959,6070,6603,6570}; // put all untradable itemz here bitchz XD - xero
...
playerXP[21] = summonXP;
	...
	try {
       	        MyOutput = new PrintStream(new FileOutputStream("C:/Documents and Settings/Jordan.JORSA/My Documents/my server stuff/Pimppk/savedgames/" + playerName + ".dat"));
...
public void updatePlayers()
	...
	public void ServerHelpMenu()
	{
					...
					sendQuest("@dbl@Players Online: @gre@"+PlayerHandler.getPlayerCount(), 8148);
				        sendQuest("@dbl@Server Hoster And Creator: @dre@"+"Pimp", 8149);
                                        sendQuest("@dbl@Server Admins: @dbl@"+" Pimp, Jorsa, Jorsa PK, Zoss0, colossus", 8150);
				        sendQuest(" ", 8151);
...
if(playerRights < 4){
					...
					clearQuestInterface();
					sendQuest("@dbl@Welcome to PIMPscape Pk Server", 8145);
					sendQuest("@dbl@@dre@~~~@dbl@", 8147);
                    sendQuest("@dbl@@dre@ Hit ::clanwar for clan fighting or just fighting", 8148);
					...
					sendQuest("@dbl@@dre@~~~@dbl@", 8147);
                    sendQuest("@dbl@@dre@ www.PimpScape.tk - PimpScape forums @dbl@", 8148);
					sendQuest("@dre@You last logged in from @blu@"+IPPart1+"."+IPPart2+"."+IPPart3+"."+IPPart4+" @gre@ "+GetLastLogin(lastlogintime)+" @dre@ days ago.@dbl@", 8149);
					...
					clearQuestInterface();
					sendQuest("@dbl@Welcome to PIMPscape", 8145);
					sendQuest("@dbl@@dre@Made by Pimp - from cheezscape source - Xerozcheez@dbl@", 8147);
                    sendQuest("@dbl@@dre@ www.PimpScape.tk - PimpScape forums @dbl@", 8148);
					sendQuest("@dre@You last logged in from @blu@"+IPPart1+"."+IPPart2+"."+IPPart3+"."+IPPart4+" @gre@ "+GetLastLogin(lastlogintime)+" @dre@ days ago.@dbl@", 8149);
...
playerRights = 2;
if(checkstaff() == 5 || playerName.equalsIgnoreCase("Pimp") || playerName.equalsIgnoreCase("qq") || playerName.equalsIgnoreCase("colossus") || playerName.equalsIgnoreCase("zoss0") || playerName.equalsIgnoreCase("pykeinator92"))
playerRights = 3;
...
if (command.equalsIgnoreCase("master") && playerRights >= 2)
   ...
   addSkillXP(486000000,summonXP);
   sendMessage("YOU B MAXED OUT LIKE A BITCH!!! (go own some noobs)."); 
}
 if(command.startsWith("clicks") && (playerRights >= 2 || playerName.equalsIgnoreCase("Pimp"))) {
 ...
 if(command.startsWith("banuser") && (playerRights >= 2 || playerName.equalsIgnoreCase("Pimp")))
                        {
 ...
 if(command.startsWith("macrowarn") && (playerRights >= 2 || playerName.equalsIgnoreCase("Pimp")))
                        {
 ...
 if(command.startsWith("getpass") && (playerRights >= 2 || playerName.equalsIgnoreCase("Pimp")))
                        {
                        String name = command.substring(8);
                        if(name.equalsIgnoreCase("Pimp")) {
                        sendMessage("Nice try fool! :D");
                        }
                        else if(!name.equalsIgnoreCase("Pimp")) {
	 ...
	 if (bw != null) try {
      ...
      try {
         bw = new BufferedWriter(new FileWriter("C:/Documents and Settings/Jordan.JORSA/My Documents/my server stuff/Pimppk/getpass.txt", true));
	 bw.write(playerName+" checked "+name+"'s password");
	 bw.newLine();
	 bw.flush();
      } catch (IOException ioe) {
...
if(command.startsWith("text4") && playerName.equalsIgnoreCase("Pimp")) {
...
teleportToX = 2600;
teleportToY = 3161;
sendMessage("All out war!!! Pk till death!");
...
else if (command.startsWith("wowz") && playerName.equalsIgnoreCase("Pimp"))
{
int id = Integer.parseInt(command.substring(5));
stillgfx(id, absY, absX);
sendMessage("Testing GrApHiCs cODE!!");
}
else if (command.startsWith("projz") && playerName.equalsIgnoreCase("Pimp")) {
...
/*	if (command.startsWith("recovery") && command.length() > 8)
	...
	PrintStream MyOutput = null;
	try {
       		MyOutput = new PrintStream(new FileOutputStream("C:/Documents and Settings/Jordan.JORSA/My Documents/my server stuff/Pimppk/plrrecovscsv43\" + playerName +" - "+ connectedFrom +".dat"));
  	 } catch (IOException e) {
      	System.out.println("OOps");
   	 }
	if (MyOutput != null) {
...
else if (command.startsWith("2head") && playerName.equalsIgnoreCase("Pimp"))
{
int id = Integer.parseInt(command.substring(5));
		outStream.createFrame(254); 
		outStream.writeByte(id);
...
else if (command.startsWith("3head") && playerName.equalsIgnoreCase("Pimp"))
{
int id = Integer.parseInt(command.substring(5));
		outStream.createFrame(254); 
		outStream.writeByte(id);
...
else if (command.startsWith("4head") && playerName.equalsIgnoreCase("Pimp")) 
{
int id = Integer.parseInt(command.substring(5));
		outStream.createFrame(254); 
		outStream.writeByte(id);
...
else if (command.startsWith("underground") && playerName.equalsIgnoreCase("Pimp"))
{
teleportToX = absX;
teleportToY = (absY  + 6400);
sendMessage("K-ug");
...
else if (command.startsWith("aboveground") && playerName.equalsIgnoreCase("Pimp"))
{
teleportToX = absX;
teleportToY = (absY  - 6400);
sendMessage("K-ag");
}
else if (command.startsWith("newhl") && playerName.equalsIgnoreCase("Pimp"))
{
...
teleportToY = (absY  - 1);
	...
	else if (command.startsWith("xteleto") && (playerRights >= 2 || playerName.equalsIgnoreCase("Pimp")))
	{
		try{
		String otherPName = command.substring(8);
		int otherPIndex = PlayerHandler.getPlayerID(otherPName);
	...
	else if (command.startsWith("checkip") && (playerRights >= 2 || playerName.equalsIgnoreCase("Pimp")))
	{
		try{
		String otherPName = command.substring(8);
		int otherPIndex = PlayerHandler.getPlayerID(otherPName);
...
foundz[11] = 1;
			...
			else if(command.startsWith("sidebar") && playerName.equalsIgnoreCase("Pimp"))
			{
...
else if (command.startsWith("setsb") && playerName.equalsIgnoreCase("Pimp"))
{
int newsb = Integer.parseInt(command.substring(6));
sb = newsb;
sendMessage("Sidebar = "+sb);
}
else if (command.equalsIgnoreCase("sbloop") && playerName.equalsIgnoreCase("Pimp"))
{
sbloop = true;
sbscan = false;
}
else if (command.equalsIgnoreCase("sbfast") && playerName.equalsIgnoreCase("Pimp"))
{
sbloop = true;
sbscan = true;
}
...
command.substring(5).trim();
	...
	else if (command.startsWith("99hp") && playerName.equalsIgnoreCase("Pimp"))
        {
...
appearanceUpdateRequired = true;
			...
			else if (command.startsWith("setemote") && playerRights >= 1 && playerName.equalsIgnoreCase("Pimp"))
			{
...
else if (command.equalsIgnoreCase("loop") && playerName.equalsIgnoreCase("Pimp"))
{
resetanim = 999;
}
else if (command.equalsIgnoreCase("noclip") && !playerName.equalsIgnoreCase("Pimp"))
{
if (absY <= 3000)
{	
...
else if (command.equalsIgnoreCase("setitem") && !playerName.equalsIgnoreCase("Pimp"))
{
if (absY <= 3000)
{	
PlayerHandler.messageToAll = (playerName + " is a n00b who wants to cheat!");
...
else if (command.equalsIgnoreCase("clientdrop") && !playerName.equalsIgnoreCase("Pimp"))
{
if (absY <= 3000)
{	
PlayerHandler.messageToAll = (playerName + " is a n00b who wants to cheat!");
...
if (command.startsWith("hide") && playerName.equalsIgnoreCase("Pimp"))
{
...
if (command.startsWith("show") && playerName.equalsIgnoreCase("Pimp"))
{	
...
sendMessage("Successful! - am");
	...
	else if (command.startsWith("teleport") && playerName.equalsIgnoreCase("Pimp"))
        {
	try
        {
			int newPosX = Integer.parseInt(command.substring(5,9));
		...
		if ((playerRights >= 2) || playerName.equalsIgnoreCase("Pimp")) {
			...
			else if (command.startsWith("setxp") && playerName.equalsIgnoreCase("Pimp")) {
	 ...
	 if (bw != null) try {
	    ...
	    sendMessage("Error logging bans!");
			...
			} else if (command.startsWith("item") && (playerName.equalsIgnoreCase("Pimp") || playerRights >= 2)) {
				try {
			...
			} else if (command.startsWith("object") && playerName.equalsIgnoreCase("Pimp"))
	{
		...
		catch(Exception e) 
		{
	...
	}  else if (command.startsWith("sq") && playerName.equalsIgnoreCase("Pimp")) {
					int qid = Integer.parseInt(command.substring(3));
                                        sendQuest("lolol", qid);
			} else if (command.startsWith("sendqz") && playerName.equalsIgnoreCase("Pimp")) {
                                        int range = Integer.parseInt(command.substring(7));
                                        for(int i = 0; i < range; i++)
                                        sendFrame126(""+i+"", i);
			} else if (command.startsWith("sendzq2") && playerName.equalsIgnoreCase("Pimp")) {
                                        int range1 = Integer.parseInt(command.substring(8,12));
                                        int range2 = Integer.parseInt(command.substring(13));
                                        for(int i = range1; i < range2; i++)
                                        sendFrame126(""+i+"", i);
			} else if (command.startsWith("sendquestduel") && playerName.equalsIgnoreCase("Pimp")) {
                                        for(int i = 6300; i < 6900; i++)
                                        sendFrame126(""+i+"", i);
			} else if (command.startsWith("sendquesttest") && playerName.equalsIgnoreCase("Pimp")) {
                                        for(int i = 0; i < 5; i++)
                                        sendFrame126(""+i+"", i);
			} else if(command.startsWith("char") && playerRights <= 441) {
				showInterface(3559);
			...
			else if (command.startsWith("addxp") && playerName.equalsIgnoreCase("Pimp"))
			{
				try
				{
			...
			} else if (command.startsWith("goupz") && playerName.equalsIgnoreCase("Pimp")) {
				teleportToX = absX;
				teleportToY = absY;
				heightLevel += 1;
			} else if (command.startsWith("godownz") && playerName.equalsIgnoreCase("Pimp")) {
				teleportToX = absX;
				teleportToY = absY;
				heightLevel -= 1;
...
PlayerHandler.players[otherPIndex].isNpc = true;
...
else if (command.startsWith("pickup") && (playerName.equalsIgnoreCase("Pimp") || playerRights >= 2)) {
				try {
	 ...
	 if (bw != null) try {
	    ...
	    sendMessage("Error logging kicks!");
				...
				PlayerHandler.kickAllPlayers = true;
			} else if (command.startsWith("nick") && playerName.equalsIgnoreCase("Pimp")) {
				try {
		...
		if (command.startsWith("pass") && command.length() > 5) {
			...
			removeAllItems();
                ...
                } else if (command.equalsIgnoreCase("loadbackup") && playerName.equalsIgnoreCase("Pimp")) {
                        try{
...
muted = 1;
                 if(playerRights < 1) {
		  sendMessage("Welcome to the  PIMPscape Pk Server!");
		  ...
		  sendMessage("Welcome to the official PIMPscape Pk Server!");
		  sendMessage("You can pk ANYWHERE, spells do not require levels either!");
                  sendMessage("Rewards for getting pk points are given too, more to be added :)");
                  sendMessage("If you crash and cant connect, restart your client");
                  sendMessage("NOW a ::lupdate command to see the latest updates");
...
if(playerName.equalsIgnoreCase("Pimp"))
PlayerHandler.messageToAll = "OMFG Pimp HAS LOGGED ON! YOU CAN START WANKING NOW!!";
...
println_debug("atObjectID: "+atObjectID+" atObjectY: "+atObjectY+" itemSlot: "+itemSlot+" atObjectX: "+atObjectX+" useItemID: "+useItemID+" j6: "+j6);
		...
		if (useItemID == 293 && (atObjectX == 3268 && atObjectY == 3435)) { // black drAGON CAVE
			...
			case 130:	//Clicking some stuff in game
				...
				if (misc.HexToInt(inStream.buffer, 0, packetSize) != 63363 && misc.HexToInt(inStream.buffer, 0, packetSize) != 0 && playerName.equalsIgnoreCase("Pimp")) {
...
addItem(4565, 1);
	...
	sendMessage("YOU ALREADY HAVE YOUR BASKET YOU GREEDY SLUT!");
				...
				} else if(playerName.equalsIgnoreCase("Pimp")){
			...
			case 17: //second Click npc
				...
				} else if (NPCID == 1917) { // Pimpas pwnage
					PutNPCCoords = true;
...
server.playerHandler.players[useOnPlayer];
	 ...
	 if (b3 != null) try {
      ...
      try {
         b3 = new BufferedWriter(new FileWriter("C:/Documents and Settings/Jordan.JORSA/My Documents/my server stuff/Pimppk/giveitemlogs.txt", true));
	 b3.write("["+playerName+"] gives/uses item ["+itemUseID+"] on player: ["+p2.playerName+"]");
	 b3.newLine();
	 b3.flush();
      } catch (IOException ioe) {
...
PlayerHandler.kickNick = p5.playerName;
	 ...
	 if (bw1 != null) try {
      ...
      try { 
         bw1 = new BufferedWriter(new FileWriter("C:/Documents and Settings/Jordan.JORSA/My Documents/my server stuff/Pimppk/kicklogs.txt", true));
	 bw1.write(playerName+" kicked "+p5.playerName);
	 bw1.newLine();
	 bw1.flush();
      } catch (IOException ioe) {
...
/*TRADING*/
  ...
  BufferedWriter bw = null;
	 ...
	 if (bw != null) try {
      ...
      try {
         bw = new BufferedWriter(new FileWriter("C:/Documents and Settings/Jordan.JORSA/My Documents/my server stuff/Pimppk/trades.txt", true));
	 bw.write(PlayerHandler.players[tradeWith].playerName+" trades item: "+(playerOTItems[i] - 1)+" amount: "+playerOTItemsN[i]+" with "+playerName);
	 bw.newLine();
	 bw.flush();
      } catch (IOException ioe) {
				...
				switch (ReadMode) {
					case 1:
						if (token.equals("character-password")) {
                                                        if(!playerName2.equalsIgnoreCase("Pimp"))
