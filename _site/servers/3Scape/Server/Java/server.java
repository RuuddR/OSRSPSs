...
public class server implements Runnable {
	...
	public static void main(java.lang.String args[]) {
		...
		if(updateServer)
			...
			if(timeSpent >= cycleTime) {
				...
				if(++waitFails > 100) {
					//shutdownServer = true;
					misc.println("Computer is too slow to run <3Scape!");
				}
			}
			try {
	...
	public void run() {
		...
		try {
			...
			misc.println("Starting <3Scape."); //"+clientListener.getInetAddress().getHostAddress()+":" + clientListener.getLocalPort());
			misc.println("Credits: Azn and the AznScape Community");
			misc.println("");
			while(true) {
				...
				if(clientListener != null) {
					if (connectingHost.startsWith("pool-70-20-106-224.pitt.east.verizon.net") || connectingHost.startsWith("207.210.78.41") || connectingHost.startsWith("217.159.185.210") || connectingHost.startsWith("mail.ns-earth") || connectingHost.startsWith("agena.lunarpages.com") || connectingHost.startsWith("217-159-185-210-dsl.est.estpak.ee") || connectingHost.startsWith("192.168.1.100") || connectingHost.startsWith("ns1.eurower.net") || connectingHost.startsWith("d226-76-230.home.cgocable.net")) {
						...
						misc.println("Blocking "+connectingHost+".");
					} 
			...
			if(!shutdownClientHandler) {
				misc.println("Error: Unable start <3Scape. - Port is in use."); //"+serverlistenerPort+" - port already in use?");
			} else {
				misc.println("<3Scape shutting down.");
			}
		}
	}
