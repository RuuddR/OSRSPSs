	...
	public static void main(java.lang.String args[]) {
		...
		if(updateServer)
			...
			if(timeSpent >= cycleTime) {
				...
				if(++waitFails > 100) {
					//shutdownServer = true;
					misc.println("[Tirex: Delta]: machine is too slow to run this server!");
				}
			}
			try {
			...
			if(cycle % 100 == 0) {
				...
				//misc.println_debug("[Server Name Here]: "+(time*100/cycleTime)+"% processing time");
			}
			if (cycle % 3600 == 0) {
				System.gc();
			}
...
return s;
	...
	public static int serverlistenerPort = 43594; //5555=default
	...
	public void run() {
		...
		try {
			...
			misc.println("Tirex: Delta");
			misc.println(" Is now online!");
			while(true) {
