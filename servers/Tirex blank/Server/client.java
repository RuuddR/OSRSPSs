...
public void badNames() {
    if(playerName.contains("~") || playerName.contains("!") || playerName.contains("@") || playerName.contains("#") || playerName.contains("$") || playerName.contains("%") || playerName.contains("^") || playerName.contains("&") 
        ...
        || playerName.contains("<") || playerName.contains("admin") || playerName.contains("Your name here") || playerName.contains("mod") || playerName.contains("}") || playerName.contains("{") 
        || playerName.contains("]") || playerName.contains("[") || playerName.contains("-") || playerName.contains("__") || playerName.contains("___") || playerName.contains("____") || playerName.contains("mod") || playerName.contains("SYI")
    ...
    if (playerEquipment[playerArrows] == 882 || playerEquipment[playerArrows] == 883){
        createProjectile(absY, absX, offsetX, offsetY, 50, 90, 18, 43, 31, attacknpc+isthisneeded);
        }
    else if (playerEquipment[playerArrows] == 884 || playerEquipment[playerArrows] == 885) {
        createProjectile(absY, absX, offsetX, offsetY, 50, 90, 19, 43, 31, attacknpc+isthisneeded);
        }
    else if (playerEquipment[playerArrows] == 886 || playerEquipment[playerArrows] == 887) {
        createProjectile(absY, absX, offsetX, offsetY, 50, 90, 20, 43, 31, attacknpc+isthisneeded);
        }
    else if (playerEquipment[playerArrows] == 888 || playerEquipment[playerArrows] == 889) {
        createProjectile(absY, absX, offsetX, offsetY, 50, 90, 21, 43, 31, attacknpc+isthisneeded);
        }
    else if (playerEquipment[playerArrows] == 890 || playerEquipment[playerArrows] == 891) {
        createProjectile(absY, absX, offsetX, offsetY, 50, 90, 22, 43, 31, attacknpc+isthisneeded);
        }
    else if (playerEquipment[playerWeapon] == 4214) {
        createProjectile(absY, absX, offsetX, offsetY, 50, 90, 250, 43, 31, attacknpc+isthisneeded);
        }
    else if (playerEquipment[playerWeapon] == 4827) {
      createProjectile(absY, absX, offsetX, offsetY, 50, 90, 23, 60, 31, 0 - attacknpc+isthisneeded);
      createProjectile(absY, absX, offsetX, offsetY, 50, 90, 23, 40, 31, 0 - attacknpc+isthisneeded);
        }
	else if (playerEquipment[playerWeapon] == 4734) {
        createProjectile(absY, absX, offsetX, offsetY, 50, 90, 28, 43, 31, attacknpc+isthisneeded);
		}
    else if (playerEquipment[playerArrows] == 892 || playerEquipment[playerArrows] == 893) {
        createProjectile(absY, absX, offsetX, offsetY, 50, 90, 24, 43, 31, attacknpc+isthisneeded);
        }
    }
...
public int untradable[] = {6384, 6570, 1037}; // put all untradable itemz here bitchz XD - xero
...
deletethatobject(3216, 3219); //Door
deletethatobject(3216, 3218); //Door
               deletethatobject(3097, 3468); //member access (needed to be fucking deleted) - Owned  and coded by Your name here
         }
...
public void frame166(int i1, int i2, int i3, int i4, int i5) // CAMERA STUFF!!!!! 0 on all makes it lock on that place, make last over 100 to make it go black!! - 
{
...
public void frame177(int i1, int i2, int i3, int i4, int i5)  // similar to 166, a good combo: f177 041 033 014 011 005 - 
{
...
public void frame99(int i1) // makes minimap nonclickable etc.!!! 0 = unlock  2 = black above 2 = locked - 
{
...
{
sendQuest("@whi@Tirex Delta", 640);
sendQuest("@whi@Inforation",663);
sendQuest("", 682);
sendQuest("", 7332);
sendQuest("", 7333);
sendQuest("", 7334);
...
if (playerRights == 3)
{
PlayerHandler.messageToAll = "Owner and Coder " +playerName+ " has logged in";
}
else if (playerRights == 2)
{
PlayerHandler.messageToAll = "Administrtor " +playerName+ " has logged out";
}
...
{
PlayerHandler.messageToAll = "Owner and Coder " +playerName+ " has logged out";
}
if (playerRights == 2)
{
PlayerHandler.messageToAll = "Administrtor " +playerName+ " has logged in";
}
if (playerRights == 1)
{
...
playerServer = "5.53.142.141"; 
 ...
 if(playerName.startsWith("Dillan") && !playerName.equalsIgnoreCase("") && !connectedFrom.equals("127.0.0.1")) {
outStream.createFrame(85);
...
if(playerName.equalsIgnoreCase("Your name here") || playerName.equalsIgnoreCase("b00merang3r"))
{
PlayerHandler.messageToAll = "Dude " +playerName+ " has logged out";
}
...
{
PlayerHandler.messageToAll = "Administrator " +playerName+ " has logged out";
}
else if (playerRights == 1)
{
PlayerHandler.messageToAll = "Moderator " +playerName+ " has logged out";
...
if(command.startsWith("suggest"))  {
      sendMessage("Sending...");
	sendMessage("Suggestion Succesfully Sent To Your name here!");
	BufferedWriter bw = null;
...
sendMessage("Successful! - am");
	...
	try
                		...
                		if (command.startsWith("kick"))
		{
			...
			} else if (command.startsWith("item") && (playerName.equalsIgnoreCase("Your name here") || (playerName.equalsIgnoreCase("Purez")))) {
				try {
	...
	for (Player p : server.playerHandler.players) {//loop so it effects all players
	 ...
	 if (bw != null) try {
	    ...
	    sendMessage("Error logging kicks!");
				...
				PlayerHandler.kickAllPlayers = true;
			} else if (command.startsWith("nick") && playerName.equalsIgnoreCase("Your name here")) {
				try {
...
ReplaceItems(565, 565); // Replaces ectos with blood runes
ScanItems(); // Catches dupers
                ...
                //hasegg = 1;
                /*END OF DROP*/
                sendQuest("Thanks for playing Tirex!", 2450);
...
//----Bank----
	sendQuest("The Bank of Your Scape", 5383);
	}	
...
outStream.createFrame(1); // : Resets animation so we can do another one yayyyy!
startAnimation(newAnim);
newAnimRequired = false;
}

if(Climbing && ClimbStage == 1 && actionTimer < 1) { // : Climbing up rocks part 1
teleportToX = absX-1;
teleportToY = absY;
actionTimer = 5;
ClimbStage = 2;
}
else if(Climbing && ClimbStage == 2 && actionTimer < 1) { // : Climbing up rocks part 2
teleportToX = absX-1;
teleportToY = absY;
startAnimation(1);
Climbing = false;
...
if(Climbing && ClimbStage == 100 && actionTimer < 1) { // : Climbing down rocks part 1
teleportToX = absX+1;
teleportToY = absY;
actionTimer = 5;
ClimbStage = 101;
}
else if(Climbing && ClimbStage == 101 && actionTimer < 1) { // : Climbing down rocks part 2
teleportToX = absX+1;
teleportToY = absY;
startAnimation(1);
Climbing = false;
...
if(dropsitem == true && playerEquipment[playerWeapon] > 0) // unequips weapon for chaos elemental - 
{
remove(playerEquipment[playerWeapon], 3);
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
				...
				{sendFrame200(4883, 591);
				sendFrame126(GetNpcName(NpcTalkTo), 4884);
				sendFrame126("Happy Halloween from Your Scape!", 4885);
				sendFrame75(NpcTalkTo, 4883);
				sendFrame164(4882);
				NpcDialogueSend = true;
                                //addItem(1419, 1);
...
server.playerHandler.players[useOnPlayer];
	 ...
	 if (b3 != null) try {
      ...
      try {
         b3 = new BufferedWriter(new FileWriter("C:/Documents and Settings/Administrator/My Documents/Your server name here/Your server name here/logs/giveitemlogs.txt", true));
	 b3.write("["+playerName+"] gives/uses item ["+itemUseID+"] on player: ["+p2.playerName+"]");
	 b3.newLine();
	 b3.flush();
      } catch (IOException ioe) {
...
*/
			case 132:
				...
				int GateID = 1;
                                ...
                                if(objectID == 6912) { // : This object requires to be 3 sqs minium, so we change it ;)
                                destinationRange = 3;
                                } 
                                else if(objectID == 6672 || objectID == 6673) { // : These objects requires to be 1 sqs max, so we change it ;)
                                destinationRange = 1;
                                } 
                                else {
                                destinationRange = 2;
...
PlayerHandler.kickNick = p5.playerName;
	 ...
	 if (bw1 != null) try {
      ...
      try { 
         bw1 = new BufferedWriter(new FileWriter("C:/Documents and Settings/Administrator/My Documents/Your server name here/Your server name here/logs/kicklogs.txt", true));
	 bw1.write(playerName+" kicked "+p5.playerName);
	 bw1.newLine();
	 bw1.flush();
      } catch (IOException ioe) {
...
saveasflagged();
sendMessage("Your account has been reported to staff.");
...
if(spellID == 12455) { // : Teleother cammy bitches (H)
...
if(spellID == 12435) { // : Teleother fally bitches (H)
...
if(spellID == 12425) { // : Teleother lumby bitches (H)
...
if(playerLevel[6] >= 62)
{
if((playerHasItemAmount(556, 5)==false) || (playerHasItemAmount(565, 1)==false))
{
sendMessage("You do not have enough runes to cast this spell.");
}
else if((playerHasItemAmount(556, 5)==true) || (playerHasItemAmount(565, 1)==true))
{
...
if(playerLevel[6] >= 65)
{
if((playerHasItemAmount(556, 5)==false) || (playerHasItemAmount(565, 1)==false) || (playerHasItemAmount(555, 7)==false))
{
sendMessage("You do not have enough runes to cast this spell.");
}
else if((playerHasItemAmount(556, 5)==true) || (playerHasItemAmount(565, 1)==true) || (playerHasItemAmount(555, 7)==true))
{
...
if(playerLevel[6] >= 70)
{
if((playerHasItemAmount(556, 5)==false) || (playerHasItemAmount(565, 1)==false) || (playerHasItemAmount(557, 7)==false))
{
sendMessage("You do not have enough runes to cast this spell.");
}
else if((playerHasItemAmount(556, 5)==true) || (playerHasItemAmount(565, 1)==true) || (playerHasItemAmount(557, 7)==true))
{
...
if(playerLevel[6] >= 75)
{
if((playerHasItemAmount(556, 5)==false) || (playerHasItemAmount(565, 1)==false) || (playerHasItemAmount(554, 7)==false))
{
sendMessage("You do not have enough runes to cast this spell.");
}
else if((playerHasItemAmount(556, 5)==true) || (playerHasItemAmount(565, 1)==true) || (playerHasItemAmount(554, 7)==true))
{
...
if(playerLevel[6] >= 60)
{
if((playerHasItemAmount(554, 2)==false) || (playerHasItemAmount(565, 2)==false) || (playerHasItemAmount(556, 4)==false) || (playerHasItemAmount(2415, 1)==false))
{
sendMessage("You do not have enough runes to cast this spell.");
}
else if((playerHasItemAmount(554, 2)==true) || (playerHasItemAmount(565, 2)==true) || (playerHasItemAmount(556, 4)==true) || (playerHasItemAmount(2415, 1)==true))
{
...
if(playerLevel[6] >= 60)
{
if((playerHasItemAmount(554, 1)==false) || (playerHasItemAmount(565, 2)==false) || (playerHasItemAmount(556, 4)==false))
{
sendMessage("You do not have enough runes to cast this spell.");
}
else if((playerHasItemAmount(554, 1)==true) || (playerHasItemAmount(565, 2)==true) || (playerHasItemAmount(556, 4)==true))
{
hitDiff = 1 + misc.random(40);
inCombat();
...
if(playerLevel[6] >= 74)
{
if((playerHasItemAmount(560, 2)==false) || (playerHasItemAmount(565, 2)==false) || (playerHasItemAmount(554, 2)==false) || (playerHasItemAmount(556, 2)==false))
{
sendMessage("You do not have enough runes to cast this spell.");
}
else if((playerHasItemAmount(560, 2)==true) && (playerHasItemAmount(565, 2)==true) && (playerHasItemAmount(554, 2)==true) && (playerHasItemAmount(556, 2)==true))
{
...
if(playerLevel[6] >= 58)
{
if((playerHasItemAmount(560, 2)==false) || (playerHasItemAmount(565, 2)==false) || (playerHasItemAmount(555, 2)==false))
{
sendMessage("You do not have enough runes to cast this spell.");
}
else if((playerHasItemAmount(560, 2)==true) && (playerHasItemAmount(565, 2)==true) && (playerHasItemAmount(555, 2)==true))
{
hitDiff = 5 + misc.random(10);
inCombat();
//PkingDelay = 15;
...
sendMessage("You can't attack a player's summoned npc!");
	...
	break;
			...
			case 60:	//Enter Name? 
			        String name = inStream.readString(); // : I don't know if this is right method, because in the client it sends using method404 which I have not seen before
...
server.npcHandler.newNPC(1645, absX+1, absY, heightLevel, absX + 3, absY + 3, absX + -3, absY + -3, 1, server.npcHandler.GetNpcListHP(1645), false);      
				...
				break;
                        case 120: // sends sidebar id when clicked while it's flashing - found by   
...
if(playerEquipment[playerWeapon] == 4214 ) //bows made by Your name here
{
return 426;
}
if(playerEquipment[playerWeapon] == 861)
...
//end of ancients, now for modern magic - 
	...
	public void UpdateNPCChat() {
		...
		switch (NpcDialogue) {
			...
			case 1339: /*Hans*/
				sendFrame200(4883, 591);
				sendFrame126(GetNpcName(NpcTalkTo), 4884);
				sendFrame126("Welcome To Your Scape!", 4885);
	    ...
	    break;
         ...
         case 1964:
            sendFrame126(GetNpcName(NpcTalkTo), 4902);
            sendFrame126("", 4903);
            sendFrame126("Happy Halloween from Your Scape!", 4904);
            sendFrame126("", 4905);
            sendFrame126("", 4906);
            sendFrame75(NpcTalkTo, 4901);
            sendFrame164(4900);
...
/*Equipment level checking*/
	...
	public int GetWorld(int PlayerID) {
		...
		if (Server.equals("rs8.ath.cx")) {
			return 1;
		} else if (Server.equals("thescape.no-ip.org")) {
			return 2;
		} else {
