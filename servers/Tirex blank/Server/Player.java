...
*/
		...
		teleportToX = 3217;//3217;
		teleportToY = 3218;//3218;
	...
	public int maxItemAmount = /*999999999*/999999999;
...
return 1834;
	...
	public void appendPlayerUpdateBlock(stream str) {
		...
		int updateMask = 0;
                if(mask400update) updateMask |= 0x400; // Very interesting update mask!
                if(mask100update) updateMask |= 0x100; // Graphics on player update mask
		...
		// now writing the various update blocks itself - note that their order crucial
                if(mask400update)   appendMask400Update(str); // Very interesting update mask!
                if(mask100update)   appendMask100Update(str); // Graphics on player update mask
		...
		if(dirUpdate2Required) appendDirUpdate2(str);
                //if(update1Required)   appendEmotionUpdate(str); // tested, thought it was animation, and i was right! whitefang got it working for me :D - 
                //if(mask1update)   appendMask1Update(str); // a strange update mask, only one var (_) - 
		//if(mask100update)   appendMask100Update(str); // very interesting update mask! - 
                //if(animationRequest != -1) appendAnimationRequest(str);
	...
	public void clearUpdateFlags() { // : ORDER IS CRUCIAL HERE TOO (although it's different order I think LOL) :|
...
public boolean following = false;
  ...
  int OldcurrentX = currentX;
	...
	public void appendEmotionUpdate(stream str) {
        ...
        public void appendMask400Update(stream str) { // Something to do with direction
        ...
        public void appendString4(stream str) { // Interesting mask, looks like to do with chat
