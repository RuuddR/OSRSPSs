			...
			if(server.npcHandler.npcs[i2].followPlayer == players[i].playerId && server.npcHandler.npcs[i2] != null)
				...
				if (players[i].savefile == true) {
					if(saveGame(players[i])){
						...
						System.out.println("Unable to save for "+players[i].playerName+".");
					};
				} else {
					System.out.println("Rejecting save for "+players[i].playerName+".");
				}
...
/*EOF*/
	...
	public boolean saveGame(Player plr)
	{
		...
		try
		{
			ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("./Saved/"+tempSave.playerName+".dat"));
