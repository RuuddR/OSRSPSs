...
public abstract class Player {	
    ...
    public Player(int _playerId) {
		...
		playerEquipment[playerHat]=579;
		playerEquipment[playerCape]=14074;
		playerEquipment[playerAmulet]=6585;
		playerEquipment[playerChest]=6107;
		playerEquipment[playerShield]=-3842;
		playerEquipment[playerLegs]=542;
	...
	pBeard=-1;
        ...
        teleportToX = 3212; // 3212;
        teleportToY = 2778; // 2778;
    ...
    protected void appendPlayerAppearance(stream str) {
        ...
        int mag = (int) ((double) (getLevelForXP(playerXP[4])) * 1.5);
        int ran = (int) ((double) (getLevelForXP(playerXP[6])) * 1.5);
        int attstr = (int) ((double) (getLevelForXP(playerXP[0]))
    ...
    protected void appendPlayerChatText(stream str) {
        str.writeWordBigEndian(
                ((chatTextColor & 0xFF) << 8) + (chatTextEffects & 0xFF));
        ...
        if (newWalkCmdSteps > 0) {
            ...
            for (i = 0; i < playerFollow.length; i++) {
                if (playerFollow[i] != -1 && following == true) {
                    PlayerHandler.players[playerFollow[i]].newWalkCmdSteps = (newWalkCmdSteps
                            + 1);
                ...
                if (dir != -1 && (dir & 1) != 0) {
                    ...
                    found = true;
                }	// we didn't need to go back in time because the current position
                // already can be connected to first

                if (!found) {
                    println_debug(
                            "Fatal: couldn't find connection vertex! Dropping packet.");
                    disconnected = true;
                } else {
                    ...
                    if (dir != -1 && (dir & 1) != 0) {
                        ...
                        if (dir == -1 || (dir & 1) != 0) {
                            ...
                            if (!found) {
                                println_debug(
                                        "Fatal: Internal error: unable to determine connection vertex!"
                                                + "  wp1=(" + wayPointX1 + ", "
                                                + wayPointY1 + "), wp2=("
