...
public class Class36
{
    ...
    {
        float f1 = (float)anIntArrayArrayArray615[i][0][k] + f * (float)(anIntArrayArrayArray615[i][1][k] - anIntArrayArrayArray615[i][0][k]);
        f1 *= 0.001525879F;
        j = 34 / j;
    ...
    public float method354(int i, int j, int k, float f)
    {
        if(j != 0)
            anInt611 = -370;
        float f1 = (float)anIntArrayArrayArray614[k][0][i] + f * (float)(anIntArrayArrayArray614[k][1][i] - anIntArrayArrayArray614[k][0][i]);
        f1 *= 0.0001220703F;
    ...
    public int method355(int i, boolean flag, float f)
    {
        ...
        if(i == 0)
        {
            float f1 = (float)anIntArray616[0] + (float)(anIntArray616[1] - anIntArray616[0]) * f;
            f1 *= 0.003051758F;
        ...
        for(int k = 1; k < anIntArray613[i]; k++)
        {
            ...
            aFloatArrayArray617[i][k * 2] = aFloatArrayArray617[i][k * 2 - 1] * f4 + aFloatArrayArray617[i][k * 2 - 2] * f5;
    ...
    public void method356(int i, Class29 class29, ByteStream byteStream)
    {
        ...
        if(j != 0)
        {
            anIntArray616[0] = byteStream.method523();
            anIntArray616[1] = byteStream.method523();
            int l = byteStream.method521();
            for(int i1 = 0; i1 < 2; i1++)
            {
                for(int j1 = 0; j1 < anIntArray613[i1]; j1++)
                {
                    anIntArrayArrayArray614[i1][0][j1] = byteStream.method523();
                    anIntArrayArrayArray615[i1][0][j1] = byteStream.method523();
                }
            ...
            for(int k1 = 0; k1 < 2; k1++)
            {
                for(int l1 = 0; l1 < anIntArray613[k1]; l1++)
                    if((l & 1 << k1 * 4 << l1) != 0)
                    {
                        anIntArrayArrayArray614[k1][1][l1] = byteStream.method523();
                        anIntArrayArrayArray615[k1][1][l1] = byteStream.method523();
                    } else
                    {
            ...
            if(l != 0 || anIntArray616[1] != anIntArray616[0])
                class29.method309(byteStream, 0);
            return;
        } else
        {
